...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def improve_defense(self, state: GameState, sector: int, defense):
        ...
        # 2a) Edge fills as wall+turret pairs after turn 10
        if turn >= 10:
            for x in list(range(1, 6)) + list(range(23, 28)):
                wloc = [x, 13]
                tloc = [x, 12]
                if (tuple(wloc) not in existing
                    and state.can_spawn(WALL, wloc)
                    and state.can_spawn(TURRET, tloc)):
                    state.attempt_spawn(WALL, wloc)
                    state.attempt_spawn(TURRET, tloc)
                    return True

        # 2b) Symmetric expansion with wall+turret pairs
        for x_left in range(4, centre):
            x_right = 28 - x_left
            locL, locR = [x_left, 13], [x_right, 13]
            tL, tR     = [x_left, 12], [x_right, 12]

            left_ok  = tuple(locL) not in existing and state.can_spawn(WALL, locL)
            right_ok = tuple(locR) not in existing and state.can_spawn(WALL, locR)

            adjL = sum((nx,13) in existing for nx in (x_left-1, x_left+1))
            adjR = sum((nx,13) in existing for nx in (x_right-1, x_right+1))

            # place both sides if valid
            if left_ok and right_ok and adjL <= 1 and adjR <= 1:
                state.attempt_spawn(WALL, locL)
                if state.can_spawn(TURRET, tL):
                    state.attempt_spawn(TURRET, tL)
                state.attempt_spawn(WALL, locR)
                if state.can_spawn(TURRET, tR):
                    state.attempt_spawn(TURRET, tR)
                return True

            # otherwise place one side only if it respects the gap rule
            if left_ok and adjL <= 1:
                state.attempt_spawn(WALL, locL)
                if state.can_spawn(TURRET, tL):
                    state.attempt_spawn(TURRET, tL)
                return True
            if right_ok and adjR <= 1:
                state.attempt_spawn(WALL, locR)
                if state.can_spawn(TURRET, tR):
                    state.attempt_spawn(TURRET, tR)
                return True

        # 3) Upgrade walls after turn 7
        if turn >= 7:
            for loc in row13:
                unit = state.contains_stationary_unit(loc)
        ...
        # 4) Build only the 2nd turret row
        ...
        # 5) Seal gaps on row 13 except at centre
        if turn >= 15:
            hole = [centre, 13]
        ...
        # 6) Funnel layers from turn 20 onward
        if turn >= 20:
            layer = min(turn - 20, 2)
            start_row = start[1]
